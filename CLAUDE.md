...
- **理論 (theory)**: `TheoryTab` サブナビ（横スクロールのピル）。
  - **学ぶ**: `LessonsPage`＋`data/lessons.ts`。ゼロから分かる土台コース(音名→度数→メジャースケール→トライアド→7th→ダイアトニック→ii-V-I)。各レッスン=**説明＋例＋`LessonFretboard`の盤面実演（入力）→ 理解度チェック多肢選択＋解説（確認）→ 練習への導線(`onGoto`)**。「ちゃんと学んでからクイズ」でゼロベースでも“なんじゃこれ”にしない。理論は“読ませず”見て・確かめて練習に繋ぐ(docs/adr/0001)。
