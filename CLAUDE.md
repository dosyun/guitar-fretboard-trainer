...
│   ├── skillStore.ts            — ★スキルマップ用 per-skill 集計 (音名/度数/トライアド/コードトーン/進行/キー機能, gft-skills-v1)
...
- **練習 (practice)**: 上部に7モードトグル「基本／コードトーン／進行／トライアド／キー機能／耳トレ／ガイド音」。
  ...
  - **トライアド**: `TriadBuilder`。ルート＋メジャー/マイナー/ディミニッシュを選び、R・3・5を指板から**集めて構築**。集めた度数はチップに✓、完成で和音が鳴る。maj↔minで3度が変わる体感、誤タップは「それは◯度」と度数名で指摘。記録は `recordSkill('triad')`（スキルマップ）のみで、`useSession`/結果画面/ヒートマップ/度数統計には未連動。
...
- **理論 (theory)**: `TheoryTab` サブナビ（横スクロールのピル）。
  - **学ぶ**: `LessonsPage`＋`data/lessons.ts`。全21レッスンを7章に段階化(初級: 1.音と度数 / 2.スケール / 3.コード / 4.キーと進行、中級: 5.コード機能と発展(機能T-SD-D/セカンダリードミナント/代理コード) / 6.響きの拡張(モード/テンション)、上級: 7.上級テクニック(モード使い分け/リハーモナイズ/転調))。章→レベルは `CHAPTER_LEVEL`(初級/中級/上級)で一覧・レッスンに色付きバッジ表示。各レッスン=**説明＋例＋(多くは)`LessonFretboard`の盤面実演（入力）→ 理解度チェック多肢選択＋解説（確認）→ 練習への導線(`onGoto`)**（※進行・和声系の一部レッスンは demo を持たずテキスト＋例のみ）。進捗は `lessonProgress`(localStorage `gft-lessons-v1`)で保存し一覧に✓/「N/21完了」バー、「学んだ→次へ」で完了マーク。**章クリアで「✓ クリア」、全完走で🎉バナー**。一覧は先頭に「続きから」(未完了の最初)＋各章は `<details>` アコーディオン(章別進捗、次レッスンの章のみ既定展開)。「ちゃんと学んでからクイズ」でゼロベースでも“なんじゃこれ”にしない。理論は“読ませず”見て・確かめて練習に繋ぐ(docs/adr/0001)。
...
- **成績 (stats)**: 総合サマリ＋指板習熟度＋**スキルマップ**(`SkillMap`/`skillStore`=種類別正答率バー)＋**弱点診断**(`MistakeClinic`/`mistakeClinic.ts`=「N弦が弱い」「度数◯を探す問題の正答率が低い」＋**混同ペア「◯と◯を混同しやすい」**(誤答の選択を`AttemptInput.wrong`として記録し集計、両方を練習ドリル)と言語化)＋推移グラフ(セッション折れ線:正答率/平均反応)＋指板ヒートマップ＋苦手ポジションTOP5＋度数の弱点。コードトーン/進行の誤答は「選んだ◯は◯度（構成音ではない）」と誤選択の中身も提示。
- **設定 (settings)**: 目標変更・音ON/OFF(`audio.ts`)・**回答テンポ**(反射=自動送り/学習=手動「次へ」、`tempo.ts` `gft-tempo-v1`。useQuizが参照、手動時はAppが「次へ」ボタン表示)・#/♭切替・フレット数・「今回のスコアをリセット」/「練習履歴を削除」(`clearPracticeData`+`clearSkills`、確認あり)＋使い方ガイド（トグル）。
