...
│   ├── DiatonicPage.tsx         — ダイアトニックコード一覧＋コード進行練習
│   ├── ArpeggioPage.tsx         — コードトーンアルペジオ番号表示
│   └── HelpPage.tsx             — 使い方説明
└── App.tsx                      — 全体統合・タブ管理
```
...
`'map' | 'quiz' | 'scale' | 'caged' | 'voicing' | 'open' | 'diatonic' | 'arpeggio' | 'help'`

- **指板マップ**: 全フレット音名/度数表示。練習範囲絞り込み可。
- **クイズ**: 音名・位置・度数の3モード。
- **スケール**: 8種スケールマップ＋クイズ（ペンタ/メジャー/マイナー/ブルース/ドリアン/ミクソリドアン/オルタード）。
- **CAGED**: CAGEDフォームの指板マップ＋クイズ。
- **ボイシング**: バレーコードボイシング（6/5/4弦ルート × 6コードタイプ）。ジャズ教本ベース。
- **オープン**: オープンコード32種（7カテゴリ）。
- **ダイアトニック**: ダイアトニックコード一覧＋ジャズ定番進行練習（6進行）。
- **アルペジオ**: コードトーンアルペジオ番号表示（フレット範囲絞り込み対応）。
- **使い方**: ヘルプページ。
