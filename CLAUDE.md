...
- **成績 (stats)**: 総合サマリ＋指板習熟度＋**スキルマップ**(`SkillMap`/`skillStore`=種類別正答率バー)＋**弱点診断**(`MistakeClinic`/`mistakeClinic.ts`=「N弦が弱い」「度数◯を探す問題の正答率が低い」と言語化、音は練習導線)＋推移グラフ(セッション折れ線:正答率/平均反応)＋指板ヒートマップ＋苦手ポジションTOP5＋度数の弱点。コードトーン/進行の誤答は「選んだ◯は◯度（構成音ではない）」と誤選択の中身も提示。
