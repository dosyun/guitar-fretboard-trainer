...
- **理論 (theory)**: `TheoryTab` サブナビ（横スクロールのピル）。
  - **学ぶ**: `LessonsPage`＋`data/lessons.ts`。ゼロから分かる土台レッスン路(音名→度数→メジャースケール→トライアド→7th→ダイアトニック→ii-V-I)。各レッスン=平易な説明＋例＋「見る/練習する」導線(`onGoto`で理論サブタブor練習モードへ)＋一部は `LessonFretboard` で度数を盤面に光らせる実演。理論は“読ませず”指板・練習に繋ぐ方針(docs/adr/0001)。
